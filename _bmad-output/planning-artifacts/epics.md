...
FR49: 授权用户可对"待分配库存"状态的发货需求执行作废操作；离开待分配库存后（含"待生成采购单"、"采购中"及之后状态）不允许作废；作废需二次确认；作废后自动释放锁定库存、销售订单状态回退为"审核通过"、展示"重新生成发货需求"按钮
...
UX-DR08: FlowProgress 流程进度条组件 -- 白色圆角卡片，步骤圆形 dot（36px，done=绿色+勾号/active=蓝色+数字+光环/pending=灰色+数字），连接线（done=绿色/其他=灰色），步骤名称标签；适用发货需求 8 步流程（需求创建→分配库存→待生成采购单→采购中→备货完成→创建物流→出库发货→完成）
...
FR-A04: Epic 9 - 钉钉扫码登录与本地 JWT 签发
FR-A05: Epic 9 - 用户管理中的钉钉绑定与解绑
...
**FRs covered:** FR-A04, FR-A05, FR-A06, FR-A07, FR-A08
...
**Given** 管理员已经完成钉钉应用配置
**When** 在系统中触发“同步钉钉组织用户”
**Then** 后端调用钉钉组织用户接口拉取当前组织用户清单
**And** 把同步结果写入独立的钉钉用户池，而不是直接创建 ERP 用户

**Given** 钉钉用户池表已存在历史同步数据
**When** 再次执行同步
**Then** 系统按稳定身份键执行 upsert
**And** 更新展示信息与最近同步时间
**And** 不重复插入相同钉钉身份

**Given** 系统中存在已同步但未绑定的钉钉用户
**When** 查看钉钉用户池
**Then** 每条记录至少具备“未绑定 / 候选 / 冲突 / 已绑定”状态字段
**And** 保留手机号、邮箱、工号、昵称、部门等最小必要匹配信息

**Given** 当前阶段只允许管理员处理绑定运营
**When** 非管理员用户尝试执行同步接口
**Then** 系统返回 `403`
**And** 保持当前临时管理员规则 `currentUser.username === 'admin'`

**Given** 同步过程中出现外部接口失败或返回异常数据
**When** 同步操作结束
**Then** 系统返回统一错误响应或同步结果摘要
**And** 不泄露钉钉 access token、client secret 或原始异常堆栈
...
**Given** 钉钉用户池中存在未处理记录
**When** 系统执行匹配建议逻辑
**Then** 按手机号、邮箱、工号的唯一命中顺序尝试匹配 ERP 用户
**And** 唯一命中时标记为 `CANDIDATE`

**Given** 某个钉钉用户命中多个 ERP 用户或信息存在冲突
**When** 系统完成匹配
**Then** 该记录标记为 `CONFLICT`
**And** 保留匹配依据，供管理员判断

**Given** 某个钉钉用户未命中任何 ERP 用户
**When** 系统完成匹配
**Then** 该记录保持 `UNBOUND`
**And** 不自动开户、不自动生成 ERP 用户

**Given** 管理员确认候选绑定
**When** 系统完成正式绑定
**Then** 把绑定结果回写到 `users` 表的钉钉字段
**And** 钉钉用户池状态更新为 `BOUND`
**And** 同一钉钉身份只能绑定一个 ERP 用户
...
**Given** 管理员进入钉钉用户同步与绑定管理页面
**When** 查看列表
**Then** 可看到钉钉用户池记录、状态、匹配建议、最近同步时间
**And** 支持按状态筛选和搜索

**Given** 某条记录处于 `CANDIDATE`
**When** 管理员确认绑定
**Then** 系统完成正式绑定
**And** 列表与详情状态更新为 `BOUND`

**Given** 某条记录处于 `CONFLICT` 或 `UNBOUND`
**When** 管理员手动选择 ERP 用户并确认绑定
**Then** 系统保存正式绑定结果
**And** 给出明确成功反馈
...
**Given** 9.6、9.7、9.8 均已完成
**When** 使用真实或等效钉钉测试组织执行联调
**Then** 能覆盖“同步成功”“唯一候选确认绑定成功”“冲突待人工处理”“解绑后重新扫码失败”主链路
**And** 形成可复核的验证记录

**Given** 执行最终回归与上线检查
**When** 复核现有密码登录、扫码登录、用户管理与新同步页面
**Then** 不破坏既有认证能力
**And** 上线前检查项形成明确结论
...
**When** 查看交易单据相关枚举
**Then** `ShippingDemandStatus` 使用 `as const` 对象模式定义：`PENDING_ALLOCATION`（待分配库存）、`PENDING_PURCHASE_ORDER`（待生成采购单）、`PURCHASING`（采购中）、`PREPARED`（已备货完成）、`PARTIALLY_SHIPPED`（部分发货）、`SHIPPED`（已发货）、`VOIDED`（已作废）
...
**Given** 后端发货需求实体
**When** 查看设计
**Then** 包含 id、demand_code（自动生成）、sales_order_id（外键）、status（待分配库存/待生成采购单/采购中/已备货完成/部分发货/已发货/已作废）、created_at、updated_at、created_by、updated_by
...
**Given** 商务跟单进入发货需求详情页
**When** 页面加载完成
**Then** 页面顶部展示：① 面包屑 + 需求编号 + 状态 Tag；② FlowProgress 流程进度条（UX-DR08），8 步：需求创建→分配库存→待生成采购单→采购中→备货完成→创建物流→出库发货→完成
...
**When** 系统执行保存
**Then** 所有 SKU 行的履行类型通过 `POST /api/shipping-demands/:id/confirm-allocation` 专用动作端点持久化，不允许通用 PATCH 直接修改目标状态
**And** 履行类型为"使用现有库存"或"部分采购"（库存部分）的 SKU 行，系统按 FIFO（入库日期从早到晚）自动跨批次完成库存锁定（FR26）
**And** 若存在任一采购数量，发货需求进入"待生成采购单"（`ShippingDemandStatus.PENDING_PURCHASE_ORDER`），不得在确认分配时直接进入"采购中"或创建采购订单
**And** 若全部使用现有库存且库存锁定完成，发货需求进入"已备货完成"（`ShippingDemandStatus.PREPARED`）且销售订单推进为"备货完成"
...
**And** 货物明细表自动预填：SKU 编码/名称/可计划数量，数量仅来自 `lockedRemainingQuantity - activeLogisticsPlannedQuantity`，不得使用待采购数量或尚未收货锁定的采购缺口

**Given** 商务跟单在物流单创建表单
**When** 填写物流信息
**Then** 表单分组卡片：① 基本信息（物流供应商-必填 EntitySearchSelect、运输方式-下拉（海运/陆运/空运/快递）-必填、公司主体-必填 EntitySearchSelect、起运港-必填 EntitySearchSelect、目的港/目的地-必填 EntitySearchSelect、运抵国-必填）；② 货物明细表（预填，来源字段、SKU 和可计划数量只读，不允许扩大计划数量，可编辑箱数、每箱数量、长×宽×高 cm、毛重 kg）；③ 报关信息（是否出口报关-开关、唛头-文本，选填）；④ 备注（FR31）
**And** 物流跟踪字段（ETD/ETA/订舱号/船司·航司/船名航次/SO·提单号/实际离港日期）创建时不展示在表单中；物流单确认后，可在详情页物流跟踪卡片随时补填或更新（FR32）

**Given** 商务跟单填写完整物流单
**When** 点击"提交"
**Then** Popconfirm 确认后物流单创建成功，状态为"已确认"（`LogisticsOrderStatus.CONFIRMED`）
**And** 物流单编号自动生成

**Given** 后端物流单实体
**When** 查看设计
**Then** 包含 id、logistics_code（自动生成）、shipping_demand_id（外键）、logistics_provider_id（外键）、transport_mode（海运/陆运/空运/快递）、company_id（外键，公司主体）、departure_port_id（外键）、destination_port_id（外键）、destination_country（运抵国）、etd（预计离港日期，可空）、eta（预计到港日期，可空）、booking_number（订舱号，可空）、carrier（船司/航司，可空）、vessel_voyage（船名航次，可空）、bl_so_number（SO/提单号，可空）、actual_departure_date（实际离港日期，可空）、is_customs_declaration（布尔，是否出口报关）、shipping_mark（唛头，可空）、status（已确认/已发运/已送达/已取消）、remark、created_at、updated_at、created_by、updated_by（FR31）
...
发货需求状态根据库存分配进展（履行类型集中确认保存、采购单创建、采购收货自动锁定、物流单出库确认）自动推进，FlowProgress 进度条实时反映当前阶段，商务跟单无需手动更新状态。
