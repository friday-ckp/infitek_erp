...
FR26: Epic 5 + Epic 8 - 发货需求详情展示（Epic 5：SKU 行展示+库存状态+履行类型 UI）；库存锁定交互（Epic 8：FIFO 跨批次分配+仓库分配区）
...
FR49: Epic 5 + Epic 8 - 发货需求作废（Epic 5：作废状态定义+UI 入口）；作废逻辑执行（Epic 8：释放锁定库存+销售订单回退+重新生成按钮）
...
**NFRs addressed:** NFR-S1, NFR-S2, NFR-S3, NFR-S4, NFR-R1, NFR-R2, NFR-U1, NFR-U2
...
### Epic 5: 销售订单与发货需求管理
销售员可以创建销售订单（表单提交即确认），系统自动生成发货需求单（含 SKU 应发量和库存查询）。商务跟单可以查看发货需求列表和详情页（枢纽视图：流程进度条、Smart Button 计数器、KPI 摘要行、产品明细表含库存色彩指示和履行类型选择）。建立发货需求作为业务枢纽的 UI 框架和创建下游单据的入口。
**FRs covered:** FR21, FR22, FR23, FR24, FR26（展示部分）, FR28, FR29, FR30, FR49（UI 入口）
**NFRs addressed:** NFR-P4
**UX-DRs addressed:** UX-DR08, UX-DR09, UX-DR10, UX-DR12, UX-DR13, UX-DR18, UX-DR26, UX-DR27

### Epic 6: 采购订单与收货入库
采购员可以创建采购订单（含从发货需求预填的请购型和主动备货型），仓管可以创建收货入库单确认收货。库存核心能力在此落地：双层结构（汇总层+批次层）、期初库存录入、收货入库创建批次、请购型收货后 FIFO 自动锁定、库存变动流水记录、库存查询。采购分组预览面板（按供应商分组确认）同时落地。
...
### Epic 8: 发货需求枢纽库存决策
商务跟单可以在发货需求详情页完成完整的库存决策 -- 对每个 SKU 设置履行类型（全部采购/部分采购/使用现有库存），选择"使用现有库存"或"部分采购"时展开内联仓库分配区查看各批次明细，确认后系统按 FIFO 自动跨批次完成库存锁定。支持发货需求作废操作（释放锁定库存、销售订单状态回退、重新生成发货需求），实现"一屏决策"核心体验。
**FRs covered:** FR26（库存锁定交互）, FR49（作废逻辑执行）
**UX-DRs addressed:** UX-DR26
