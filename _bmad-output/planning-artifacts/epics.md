...
FR20: 所有列表页支持分页加载
FR21: 授权用户可通过表单创建销售订单（草稿不入库），填写完整后提交直接创建为"已确认"状态；需关联客户，添加订单行（每行关联 SKU、数量和单价），选择订单币种；订单类型字段（普通/售后/样品），MVP 仅启用普通订单
FR22: 销售订单通过表单提交直接创建为"已确认"状态，无独立的草稿→确认步骤；一经创建不支持撤销至草稿
FR23: 授权用户可查看销售订单列表（按状态、客户筛选）和订单详情
FR24: 系统在销售订单确认时自动生成发货需求单，包含所有 SKU 及应发数量；发货需求唯一来源为销售订单确认；生成时自动查询每个 SKU 可用库存
...
FR49: 授权用户可对"待分配库存"状态的发货需求执行作废操作；处于"处理中"及之后状态不允许作废；作废需二次确认；作废后自动释放锁定库存、销售订单状态回退为"已确认"、展示"重新生成发货需求"按钮
...
FR21: Epic 5 - 创建销售订单（表单提交即确认，关联客户+SKU 行项+币种+订单类型）
FR22: Epic 5 - 销售订单创建即确认，无草稿持久化
FR23: Epic 5 - 销售订单列表（按状态/客户筛选）和详情查看
FR24: Epic 5 - 销售订单确认时自动生成发货需求单（含 SKU 应发量+库存查询）
...
FR30: Epic 7 - 发货需求状态随出库确认自动更新为"已发运"
...
### Epic 6: 采购订单与收货入库
Story 6.1 首先建立库存双层数据模型（inventory_summary + inventory_batch）并落地库存查询接口和期初录入（FR39、FR40、FR43）。采购员可以创建采购订单（含从发货需求预填的请购型和主动备货型），仓管可以创建收货入库单确认收货。库存写操作在此落地：收货入库创建批次、请购型收货后 FIFO 自动锁定、库存变动流水记录。采购分组预览面板（按供应商分组确认）同时落地。
**FRs covered:** FR27, FR35, FR36, FR37, FR38, FR39, FR40, FR42, FR43, FR44, FR45
**NFRs addressed:** NFR-P5, NFR-U3
**UX-DRs addressed:** UX-DR11, UX-DR27, UX-DR29

### Epic 7: 物流单与发货出库
商务跟单可以创建物流单（基本信息+物流跟踪+装箱信息+报关信息），仓管可以从物流单创建发货出库单确认发货（含库存锁定量校验）。发货确认后库存自动扣减并释放锁定量，发货需求状态自动更新为"已发运"，销售订单状态自动更新为"已发货" -- 完成端到端业务闭环。
...
As a 销售员,
I want 通过表单创建销售订单（关联客户、添加 SKU 行项、选择币种和订单类型），提交后系统直接确认订单,
So that 订单立即生效，后续可在订单详情页手动触发生成发货需求。
...
**Given** 销售员填写完整订单表单
**When** 点击"提交"
**Then** 系统弹出 Popconfirm（UX-DR16 状态推进）："确认提交？提交后订单立即生效，无法撤回"
**And** 确认后订单创建为"已确认"状态（蓝色标签），订单编号自动生成（FR22）
**And** 提交响应时间 < 2 秒（NFR-P4）

**Given** 销售订单成功创建为"已确认"状态
**When** 后端确认逻辑执行
**Then** 前端跳转至订单详情页，展示 Notification.success（UX-DR17 重要成功）："订单已确认"
**And** 详情页操作区展示"生成发货需求"按钮（Primary 蓝色实心），等待用户手动触发

**Given** 后端销售订单实体
**When** 查看设计
**Then** 包含 id、order_code（自动生成）、customer_id（外键）、currency_id（外键）、order_type（普通/售后/样品）、status（已确认/已发货）、total_amount（自动汇总）、remark、created_at、updated_at、created_by、updated_by
...
**Given** 后端发货需求实体
**When** 查看设计
**Then** 包含 id、demand_code（自动生成）、sales_order_id（外键）、status（待分配库存/处理中/已发运/已作废）、created_at、updated_at、created_by、updated_by
...
As a 商务跟单,
I want 在发货需求详情页直接创建物流单，以及对"待分配库存"状态的发货需求执行完整的作废操作（释放已锁定库存、销售订单状态回退、支持重新生成）,
...
**Then** Modal 提示文案："确认作废此发货需求？作废后将释放已锁定库存，销售订单状态回退为'已确认'，操作不可撤销"
...
**Given** 发货需求状态为"处理中"或之后状态
**When** 查看详情页
**Then** 不展示"作废"按钮（FR49 规则：处理中及之后不允许作废）

## Epic 6: 采购订单与收货入库

采购员可以创建采购订单（含从发货需求预填的请购型和主动备货型），仓管可以创建收货入库单确认收货。库存核心能力在此落地：双层结构（汇总层+批次层）、期初库存录入、收货入库创建批次、请购型收货后 FIFO 自动锁定、库存变动流水记录、库存查询。

**FRs covered:** FR27, FR35, FR36, FR37, FR38, FR39, FR40, FR42, FR43, FR44, FR45
**NFRs addressed:** NFR-P5, NFR-P6, NFR-U3
**UX-DRs addressed:** UX-DR11, UX-DR27, UX-DR29

### Story 6.1: 库存双层结构、查询接口与期初录入

> **技术前置（本 Story 的第一项实现任务）**：建立库存双层数据模型并运行迁移脚本。
> - 汇总层实体 `inventory_summary`：id、sku_id（FK）、warehouse_id（FK）、actual_quantity（默认 0）、locked_quantity（默认 0）、available_quantity（计算列 = actual - locked）、updated_at
> - 批次层实体 `inventory_batch`：id、sku_id（FK）、warehouse_id（FK）、batch_quantity、batch_locked_quantity、source_type（期初录入/采购入库）、source_document_id、receipt_date（FIFO 排序依据）、created_at、updated_at
> - InventoryModule 包含：inventory.module.ts、两个 entity 文件、inventory.service.ts、inventory.controller.ts
> - 汇总层与批次层须保持一致（FR39）

As a 系统管理员,
I want 系统以双层结构（汇总层+批次层）维护库存，并支持录入期初库存数量用于系统上线初始化，同时提供库存查询接口供发货需求生成时调用,
So that 系统上线时有准确的库存基础数据，发货需求生成时可实时查询 SKU 可用库存，后续所有库存变动在统一结构上操作。

**Acceptance Criteria:**

**Given** 系统管理员进入期初库存录入页面
**When** 选择 SKU（EntitySearchSelect）和目标仓库（EntitySearchSelect），填写期初数量（正整数）
**Then** 提交后在批次层创建"期初录入"来源的批次记录
**And** 汇总层对应 SKU+仓库的 actual_quantity 更新为期初数量（FR40）

**Given** 系统管理员对同一 SKU+仓库重复提交期初录入
**When** 提交新数量
**Then** 系统覆盖之前的期初批次记录和汇总层数据（FR40 支持重复提交覆盖）

**Given** 授权用户调用库存查询接口
**When** GET `/api/inventory/available?skuIds=1,2,3&warehouseId=1`（发货需求生成时调用）
**Then** 返回 `{ success: true, data: [{ skuId, warehouseId, actualQuantity, lockedQuantity, availableQuantity }] }`（FR43）
**And** 若某 SKU+仓库 尚无 inventory_summary 记录，则返回 `{ availableQuantity: 0 }`（不报错）
**And** 响应时间 < 1 秒（NFR-P6）

**Given** 授权用户进入库存查询页面
**When** 按 SKU 或仓库查询
**Then** 展示当前库存数量：实际库存、锁定量、可用库存（NFR-P6 响应 < 1 秒）
**And** 支持按 SKU 搜索、按仓库筛选、分页加载（FR43）

**Given** 库存并发操作
**When** 多个操作同时修改同一 SKU+仓库的库存
**Then** 使用 TypeORM QueryRunner 事务 + SELECT ... FOR UPDATE 行锁保证一致性
**And** 死锁时指数退避最多 3 次重试，超限抛 ConflictException({ code: 'CONCURRENT_UPDATE' })
...
As a 系统,
I want 出库确认后自动更新关联物流单状态为"已出库"，发货需求状态为"已发运"，销售订单状态为"已发货",
...
**Then** 当发货需求关联的所有物流单均为"已出库"时，发货需求状态更新为"已发运"（FR25）

**Given** 发货需求状态更新为"已发运"
**When** 后端检查关联销售订单
**Then** 销售订单状态更新为"已发货"（FR25）
...
**Then** 发货需求状态从"待分配库存"**自动**更新为"处理中"
**And** FlowProgress 进度条推进至"采购备货"步骤（若有需采购的 SKU）或"库存锁定"步骤（若所有 SKU 均走库存）

**Given** 发货需求处于"处理中"，所有 SKU 行均完成库存锁定（"使用现有库存"已在确认分配时锁定 + "请购型"采购订单收货后自动锁定）
**When** 后端检查所有行锁定状态（在任意锁定操作完成后触发检查）
**Then** FlowProgress 进度条推进至"库存锁定"步骤
