...
| 阶段 | 场景 | 期望情感 | 需避免的情感 |
|------|------|---------|------------|
| 首次使用 | 登录系统，可选择账号密码或钉钉扫码登录，进入后看到导航结构 | 清晰、亲切 -- "这跟我的工作流对得上" | 迷茫 -- "这么多菜单从哪开始" |
| 管理绑定 | 管理员同步钉钉组织用户并处理待绑定列表 | 可控、踏实 -- "系统已经帮我找好了大部分候选" | 焦虑 -- "我要自己去查一堆身份字段" |
...
### DingTalk Binding Operations

- 用户管理页继续展示 ERP 用户的钉钉绑定状态、绑定时间，并保留绑定/解绑兜底入口
- 新增“钉钉用户同步与绑定管理”后台工作区，供管理员执行组织同步与处理待绑定用户
- 页面至少支持以下能力：
  - 一键手动同步钉钉组织用户
  - 查看最近同步时间与同步结果摘要
  - 按状态筛选：未绑定 / 候选 / 冲突 / 已绑定
...
- **面包屑连续性**：列表页、新建页、详情页、编辑页都必须持续显示顶栏面包屑，动态路由（如 `/:id`、`/:id/edit`）不得导致面包屑丢失
- **唯一性**：顶栏面包屑是页面唯一的路径导航，内容区内不再重复显示第二套面包屑
- **导航同步**：侧边栏当前模块/子功能高亮与顶栏面包屑必须基于同一套路由归属规则，进入详情页或编辑页后仍需保留原模块上下文
