...
- [x] 创建 UnitModule 7 文件结构
  - [x] `apps/api/src/modules/master-data/units/units.module.ts`
  - [x] `apps/api/src/modules/master-data/units/units.controller.ts`
  - [x] `apps/api/src/modules/master-data/units/units.service.ts`
  - [x] `apps/api/src/modules/master-data/units/units.repository.ts`
  - [x] `apps/api/src/modules/master-data/units/entities/unit.entity.ts`
  - [x] `apps/api/src/modules/master-data/units/dto/create-unit.dto.ts`
  - [x] `apps/api/src/modules/master-data/units/dto/update-unit.dto.ts`
  - [x] `apps/api/src/modules/master-data/units/dto/query-unit.dto.ts`
  - [x] `apps/api/src/modules/master-data/units/tests/units.service.spec.ts`

- [x] 创建 Unit Entity（继承 BaseEntity，含软删除）

- [x] 创建 QueryDto 基类（keyword + page + pageSize）
  - [x] 放在 `apps/api/src/common/dto/base-query.dto.ts`（供后续所有模块复用）

- [x] 实现 UnitsService（CRUD + 分页查询）

- [x] 实现 UnitsController（4 个标准端点）

- [x] 创建 TypeORM Migration 文件（`units` 表）

- [x] 在 AppModule 注册 UnitsModule

- [x] 编写 Service 单元测试

### 前端实现

- [x] 创建 units 页面目录 `apps/web/src/pages/master-data/units/`
  - [x] `index.tsx`（列表页，使用 ProTable）
  - [x] `detail.tsx`（详情页，使用 ProDescriptions）
  - [x] `form.tsx`（表单页，使用 ProForm）

- [x] 创建 API 层 `apps/web/src/api/units.api.ts`
...
- [x] [Review][Decision] 缺少 DELETE 端点 — 按 Story AC 与任务清单仅要求 4 个端点（GET列表/GET详情/POST/PATCH），为避免实现未定义功能，本 Story 不补充 DELETE，后续如架构文档新增要求再单独开 Story

### 需要修复的问题

- [x] [Review][Patch] API 响应格式不一致 [apps/web/src/api/units.api.ts:26] — 前端 `UnitsListData` 期望 `list` 字段，但后端 `UnitsRepository.findAll()` 返回 `data` 字段，导致前端无法正确解析数据
- [x] [Review][Patch] 类型不匹配：Unit.id [apps/web/src/api/units.api.ts:5] — 前端定义 `id: string`，但后端返回 `number` 类型，会导致类型检查失败
- [x] [Review][Patch] QueryUnitDto 缺少 status 过滤字段 [apps/api/src/modules/master-data/units/dto/query-unit.dto.ts] — 前端列表页支持 status 筛选，但 DTO 中没有 `status` 字段，导致筛选功能无效
- [x] [Review][Patch] 前端 API 调用缺少错误处理 [apps/web/src/api/units.api.ts] — `getUnits()` 等函数没有 try-catch，错误会直接抛出导致应用崩溃
- [x] [Review][Patch] Migration 缺少 down() 方法 [apps/api/src/migrations/20260419000100-create-units-table.ts] — 只有 `up()` 没有 `down()`，无法回滚迁移
- [x] [Review][Patch] Entity 缺少 @Unique() 装饰器 [apps/api/src/modules/master-data/units/entities/unit.entity.ts] — 虽然 Migration 创建了唯一索引，但 Entity 应该用 `@Unique()` 装饰器声明
- [x] [Review][Patch] 前端表单页缺少字段验证 [apps/web/src/pages/master-data/units/form.tsx] — 需要添加必填、长度限制等验证规则
- [x] [Review][Patch] 前端列表页缺少错误状态处理 [apps/web/src/pages/master-data/units/index.tsx:65] — useQuery 的错误状态未处理，API 错误会导致页面空白
- [x] [Review][Patch] 前端列表页分页状态未重置 [apps/web/src/pages/master-data/units/index.tsx:50] — 当 keyword 或 status 变化时，应重置 page 为 1，否则用户会看到空结果
...
- [x] 后端：`/api/units` 的 GET（列表+分页）、GET/:id（详情）、POST（创建）、PATCH/:id（编辑）均可正常调用
- [x] 后端：Migration 文件已创建，`units` 表结构正确
- [x] 后端：Service 单元测试通过
- [x] 前端：列表页使用 ProTable，满足 UX-DR04 所有要素（搜索、筛选 Tag、分页、空状态、加载状态）
- [x] 前端：详情页使用 ProDescriptions，满足 UX-DR05
- [x] 前端：表单页使用 ProForm，满足 UX-DR06（必填星号、分组卡片）
- [x] 前端：状态使用彩色 Tag（色彩 + 文字双重编码），满足 UX-DR07
- [x] 前端：所有 API 调用通过 `src/api/` 层 + TanStack Query
- [x] 路由和导航已注册
