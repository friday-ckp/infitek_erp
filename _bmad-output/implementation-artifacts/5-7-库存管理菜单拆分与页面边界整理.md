...
- [x] [Review][Patch] 期初库存保存后未失效独立流水页查询缓存 [`apps/web/src/pages/inventory/opening-balances.tsx:53`] — 已补充 `inventory-transactions-page` query key 失效，避免保存后立即进入独立流水页时短暂看到旧缓存。
- [x] [Review][Patch] 独立查询页缺少页面内持久错误态 [`apps/web/src/pages/inventory/available.tsx:263`, `apps/web/src/pages/inventory/batches.tsx:262`, `apps/web/src/pages/inventory/transactions.tsx:318`] — 已为可用库存、批次库存明细和库存变动流水补充 `Alert` 错误态，满足 AC4/AC8 的页面反馈要求。
...
- 2026-05-27: Code review completed; fixed independent transaction ledger cache invalidation after opening-balance save and added persistent query error states to all independent inventory query pages.
- 2026-05-27: `pnpm --filter web build` passed after code review fixes.
...
- Code review findings resolved: independent transaction ledger cache is invalidated after opening-balance save; available inventory, batch details, and transaction ledger now show page-level error states.
...
| 2026-05-27 | 1.1 | 完成代码审查修复并将 Story 状态推进到 done | GPT-5 Codex |
