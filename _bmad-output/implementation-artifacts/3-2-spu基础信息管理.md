...
**Given** 后端 SPU API
**When** 查看实体设计
**Then** SPU 实体包含：id、spu_code（自动生成 SPU001 格式）、name、category_id、category_level1_id、category_level2_id、unit、manufacturer_model、customer_warranty_months、purchase_warranty_months、supplier_warranty_note、forbidden_countries、invoice_name、invoice_unit、invoice_model、supplier_name、company_id、category_level1_code、category_level2_code、category_level3_code、created_at、updated_at、created_by、updated_by
...
export class Spu extends BaseEntity {
  ...
  @Expose()
  categoryLevel1Code: string | null; // 一级分类编号（冗余）

  @Column({ name: 'category_level2_code', type: 'varchar', length: 20, nullable: true })
  @Expose()
  categoryLevel2Code: string | null; // 二级分类编号（冗余）
...
// category_level1_code(varchar 20, nullable)
// category_level2_code(varchar 20, nullable)
// category_level3_code(varchar 20, nullable)
...
- apps/api/src/migrations/20260422000100-spu-add-category-codes.ts（新建，CC 补丁）
