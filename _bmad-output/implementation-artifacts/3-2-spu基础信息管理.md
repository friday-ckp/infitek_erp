...
- [ ] 创建 Entity `apps/api/src/modules/master-data/spus/entities/spu.entity.ts`
- [ ] 创建 DTO：`create-spu.dto.ts`、`update-spu.dto.ts`、`query-spu.dto.ts`
- [ ] 创建 Repository `spus.repository.ts`（含 findAll、findById、findByCode、create、update、delete、generateCode）
- [ ] 创建 Service `spus.service.ts`（含 CRUD、分类校验、code 自动生成）
- [ ] 创建 Controller `spus.controller.ts`（标准 CRUD 5 个端点）
- [ ] 创建 Module `spus.module.ts`（import ProductCategoriesModule，export SpusService）
- [ ] 创建 Migration `20260421000100-create-spus-table.ts`
- [ ] 注册 SpusModule 到 `apps/api/src/app.module.ts`
- [ ] 创建 Service 单元测试 `tests/spus.service.spec.ts`

### 前端：spus 页面

- [ ] 创建 API 层 `apps/web/src/api/spus.api.ts`
- [ ] 创建列表页 `apps/web/src/pages/master-data/spus/index.tsx`
- [ ] 创建详情页 `apps/web/src/pages/master-data/spus/detail.tsx`
- [ ] 创建表单页 `apps/web/src/pages/master-data/spus/form.tsx`
...
*Story 由 BMad Create-Story Agent 创建 — 2026-04-21*
